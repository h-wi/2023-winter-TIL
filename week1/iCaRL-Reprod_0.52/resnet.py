...
__all__ = ['ResNet', 'resnet18', 'resnet32','resnet34', 'resnet50', 'resnet101',
...
class ResNet(AbstractCNN):
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
        ...
        self.inplanes = planes * block.expansion #여기서 self.inplane 알아서 업데이트
...
class ICaRLNet(ResNet):
    def __init__(
        ...
        # first stack of residual blocks > 16 x 32 x 32
        self.layer1 = self._make_layer(block, 16, layers[0], stride=1)

        # second stack of residual blocks > 32 x 16 x 16
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2)

        # third stack of residual blocks > 64 x 8 x 8
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2)

        self.avgpool=nn.AdaptiveAvgPool2d((1,1))

        self.embed_dim = 64 * block.expansion
        self.head = None
    ...
    def _forward_impl(self, x: Tensor) -> Tensor: #head가 Classifier로 붙여진 상황에서, 현재 클래스에 대해서 Crossentropy(Softmax)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x = self.head(x)
        # print(f'after classfier : {x.shape}')
        return x
    ...
    def forward_tokens(self, x):
        ...
        return x.view(x.shape[0], self.embed_dim, -1).permute(0, 2, 1), [f1, f2, f3]

    def forward_features(self, x): #rehearsal method에서 각 layer의 featuremap이 필요할 때 사용
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
...
def _icarlnet(
    ...
    **kwargs: Any
    ...
    model = ICaRLNet(block, layers, **kwargs)
    if pretrained:
        ValueError("ICaRLNet does not support pre-trained model")
    return model
...
#Make ResNet-32 for iCaRL
def resnet32(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ICaRLNet:
    r"""ResNet-32
    내가 만듬
    """
    return _icarlnet('resnet32', BasicBlock, [5, 5, 5], pretrained, progress,
                   **kwargs)
