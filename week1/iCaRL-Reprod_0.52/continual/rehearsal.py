    ...
    with torch.no_grad():
        for x, y, _ in loader:
            if hasattr(model, 'module'):
                feats, _, _ = model.module.forward_features(x.cuda()) #TO-DO: all과 last의 차이점 확인하기
            else:
            ...
            if isinstance(feats, list):
                ...
                if ensemble_handling == 'last': # 가장 마지막 배치의 가장 마지막 feature를 사용
                    feats = feats[-1]
                elif ensemble_handling == 'all': # 얜 뭘까? 질문
